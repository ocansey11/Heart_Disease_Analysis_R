...
# Looking at the graph we cannot make a general statement that Males are likely to suffer from heart diseases,
...
# levels(heart_clean$sex) <-c("Female", "Male")
...
# Split the Data
# We are going to use the initial Heart dataframe since the levels are numeric
...
accuracy_results
# You can decide which pvalue cutoff to use. depending on F meas and cm sensitivity 0.6 seems to be the best.



  
  # 2. IMPROVING ON OUR PREVIOUS MODEL USING LOISTIC REGRESSION AND  K-FOLD CROSS VALIDATION 
...
# This was one of the graph with little to no correlation with our target variable, hence i decide to see what happens if i omit it from our model
  ...
  #  The accuracy increases to 0.8844221 and an F_meas of  0.88 
  #  I was curious to see the effect of cholesterol as well, because i felt it had little correlation looking at its graph, 
  # but when i removed the cholesterol variable the accuracy dropped to 0.879397, so i brought it back in and left it at that  
  ...
  # USING KNN
  model_knn <- train(target ~ thal + restecg + cp  + exang + trestbps + age + sex + fbs + ca + oldpeak + slope + thalach, data = train_set,  method = "knn", trControl = train_control)
  y_hat_knn <- predict(model_knn, test_set)
  cm_knn <-  confusionMatrix(y_hat_knn,test_set$target)
  cm_knn$overall["Accuracy"]
  im_knn <- varImp(model_knn)
  im_knn
  # We notice fbs had 0.00 varImportance hence we run the Knn model again without it
  
  
  model_knn <- train(target ~ thal + restecg + cp  + exang + trestbps + age + sex + thal + restecg + cp  + ca + oldpeak + slope + thalach, data = train_set,  method = "knn", trControl = train_control)
  y_hat_knn <- predict(model_knn, test_set)
  cm_knn <-  confusionMatrix(y_hat_knn,test_set$target)
  cm_knn$overall["Accuracy"]
  im_knn <- varImp(model_knn)
  ...
  # We notice restecg had 0.00 var Importance as well. There seems to be a trend. Also restecg had a very low vaImp of 0.3171.
  # We notice after removing sex and age we get a better accuracy. However it is lower than the accuracy from glm model.
  # I continued to chip away predictors until i had an accuracy of 0.8442211. 
  
  
  model_knn <- train(target ~  oldpeak + thalach + ca + thal + slope, data = train_set,  method = "knn", trControl = train_control)
  y_hat_knn <- predict(model_knn, test_set)
  cm_knn <-  confusionMatrix(y_hat_knn,test_set$target)
  cm_knn$overall["Accuracy"]
  ...
  # We notice slope has a varImp of 0, however if we remove the predictor the total accuracy drops.
  ...
  # Rerun the model without fbs, slope, age, sex and restecg
  tree_model <- rpart(target ~  thal +  cp  + ca + oldpeak +  thalach, data = train_set, method = "class")
  varImp(tree_model)
  
  plotcp(tree_model)
  plot(tree_model, uniform=TRUE,
       main="Classification Tree for Heart Disease")
  text(tree_model, use.n = TRUE, all=TRUE, cex=.9)
